...
class OPT_attack_sign_SGD(object):
    ...
    def attack_untargeted(self, x0, y0, alpha = 0.2, beta = 0.001, iterations = 1000, query_limit=20000,
        ...
        ####################################### gradint ########################################
        path_mode = 'rand'
        path_gradint = torch.zeros(x0.shape)
        batch_size = x0.size()[0]
        imagesize = x0.size()[2]
        x0_np = x0.cpu().numpy()     
        path_axis = []    
        path_mask = torch.zeros(batch_size, 3, imagesize, imagesize)
        sali_map =  test. get_saliencyMaps(x0,batch_size)
        patchsize = 32

        if path_mode == 'rand':
            for m in range (batch_size): #        
                x= x0[m]
                #r = torch.randperm(imagesize - patchsize)
                #r,c = torch.randperm(imagesize - patchsize,imagesize - patchsize)
                pathindex = np.random.randint(1,(imagesize - patchsize),size=((imagesize - patchsize)*(imagesize - patchsize),2))
                mask = torch.ones(1, 3, imagesize, imagesize).cuda()

                for i in range((imagesize - patchsize)*(imagesize - patchsize)):
                    r,c = pathindex[i]
                    mask[:,:,r:r + patchsize-1, c:c + patchsize-1] = 0
                    query_count += 1
                    X = torch.mul(x,mask)
                    if model.predict_label(X.cuda()) != y0:
                        path_axis.append((r,c))   
                        path_mask[m,:,r:r + patchsize -1, c:c + patchsize -1] = 1
                        break
        else:    
            for k in range (batch_size): #
                ...
                rowsize = np.shape(img)[0]
                conlsize = np.shape(img)[1]

                ##矩阵列累加、相减运算

                img_grad[img_grad > 0] = 1
                #img_grad = np.multiple(img_grad, sali_map[k].numpy())   ###去除非显著性区域的梯度值
                img_grad = img_grad * sali_map[k].numpy()      ###去除非显著性区域的梯度值
                for i in range(1,conlsize):    
                    img_grad[:,i] = img_grad[:,i] + img_grad[:,i-1]

                image_conl_sub = np.zeros(np.shape(img_grad)[0:2])
                image_conl_sub[:,0] = img_grad[:,patchsize-1]     #列相减时首列补零
                for i in range(1,conlsize - patchsize + 1):
                    image_conl_sub[:,i] = img_grad[:,i+patchsize-1] - img_grad[:,i-1]        

                ##矩阵行累加、相减运算
                for i in range(1,rowsize):
                    image_conl_sub[i,:] = image_conl_sub[i,:] + image_conl_sub[i-1,:]

                image_row_sub = np.zeros(np.shape(img)[0:2])
                image_row_sub[0,:] = image_conl_sub[patchsize-1,:]     #行相减时首行补零
                for i in range(1,rowsize - patchsize + 1):
                    image_row_sub[i,:] = image_conl_sub[i+patchsize-1,:] - image_conl_sub[i-1,:]        

                image_conl_row = image_row_sub[0:rowsize-patchsize+1,0:conlsize-patchsize+1] #列相减后矩阵列维度减小
                #image_conl_row = torch.from_numpy(image_conl_row)
                #path_gradint[i] = image_conl_row


                max_weight = np.argmax(image_conl_row)
                r,c = divmod(max_weight,image_conl_row.shape[1])
                mask = torch.ones(1, 3, imagesize, imagesize).cuda()
                mask[:,:,r + patchsize, c:c + patchsize] = 0
                x= x0[k]

                query_count += 1
                X = torch.mul(x,mask)
                while model.predict_label(X.cuda()) == y0:
                    query_count += 1
                    image_conl_row[r][c] = 0
                    max_weight = np.argmax(image_conl_row)
                    if image_conl_row.max()==0:
                        break
                    r,c = divmod(max_weight,image_conl_row.shape[1])
                    mask[:,:,r + patchsize -1 , c:c + patchsize -1] = 0
                    X = torch.mul(x, mask)

                path_axis.append((r,c))   
                path_mask[k,:,r:r + patchsize -1 , c:c + patchsize -1] = 1
        ########################################################################################
        ...
        for i in range(num_directions):
            ...
            theta = torch.mul(theta, path_mask)
            X = x0+torch.tensor(theta, dtype=torch.float).cuda()
