...
class OPT_attack_sign_SGD(object):
    ...
    def attack_untargeted(self, x0, y0, alpha = 0.2, beta = 0.001, iterations = 1000, query_limit=20000,
        ...
        totalzeronum += (150528 - best_zero_num)
        ...
        for i in range(iterations):   
            ...
            for _ in range(15):
                ...
                ls_count += count                 
                L1 = LA.norm(new_g2*new_theta)
            #################################        
                '''new_theta, count = self.fine_grained_dimension_search_local(
                    model, x0, y0, new_theta, initial_lbd = new_g2, tol=beta/500)                    
                ls_count += count
                L2 = LA.norm(new_g2*new_theta) 

                new_g2, count = self.fine_grained_binary_search_local(
                    model, x0, y0, new_theta, initial_lbd, tol=beta/500)
                ls_count += count'''
            ################################# 
        ...
        if distortion is None or gg < distortion:
            ...
            print("Succeed distortion {:.4f} target"
                  " {:d} queries {:d} LS queries {:d}\n".format(gg, target.item(), query_count, ls_total))
            return x0 + torch.tensor(gg*xg, dtype=torch.float).cuda(), gg, True, query_count, xg


        '''##############################################################
        new_theta, count, zero_num = self.fine_grained_dimension_search_local(
                    model, x0, y0, xg, initial_lbd = gg, tol=beta/500)                    
        ls_count += count
        xg = new_theta

        totalzeronum = gl.get_value('total_zero_num')
        totalzeronum += zero_num
        gl.set_value('total_zero_num', totalzeronum)
    ...
    def fine_grained_dimension_search_high(self, model, x0, y0, theta, initial_lbd = 1.0, tol=1e-5):
        nquery = 0
        if model.predict_label(x0+torch.tensor(initial_lbd*theta, dtype=torch.float).cuda()) != y0:
            '''batch,channel,row,conl = np.unravel_index(np.argmax(theta),theta.shape)
            new_theta = theta
            new_theta[batch,channel,row,conl] = 0
            while model.predict_label(x0+torch.tensor(initial_lbd*new_theta, dtype=torch.float).cuda()) != y0:
                theta = new_theta
                batch,channel,row,conl = np.unravel_index(np.argmax(theta),theta.shape)
                #new_theta = theta
                new_theta[batch,channel,row,conl] = 0
